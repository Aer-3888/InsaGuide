...
## 1.1 Experiences aleatoires et espaces echantillonnaux

### Definitions

Une **experience aleatoire** est une experience dont le resultat ne peut etre predit avec certitude en raison du hasard.

- **Espace echantillonnal** $\Omega$ : L'ensemble de tous les resultats possibles (aussi appele univers ou ensemble fondamental).
- **Evenement** $A$ : Un sous-ensemble de $\Omega$. Un evenement "se realise" lorsque le resultat $\omega \in A$.
- **Evenement elementaire** : Un singleton $\{\omega\}$ contenant un seul resultat.

Les espaces echantillonnaux **discrets** sont au plus denombrables ($\subseteq \mathbb{N}$). Les espaces **continus** sont non denombrables ($\subseteq \mathbb{R}$).
...
La **tribu borelienne** sur $\mathbb{R}$ est la plus petite tribu contenant tous les intervalles ouverts $]-\infty, a[$.

**Exemple** : Pour $\Omega = \{a, b, c\}$, la tribu triviale est $\{\emptyset, \Omega\}$. L'ensemble des parties $\{\emptyset, \{a\}, \{b\}, \{c\}, \{a,b\}, \{a,c\}, \{b,c\}, \Omega\}$ est aussi une tribu valide.

---

## 1.2 Axiomes des probabilites (Kolmogorov)
...
A partir des axiomes, on obtient :
...
On repete l'experience $N$ fois et on observe la frequence de $A$ :

$$P(A) = \lim_{N \to \infty} \frac{\text{nombre de fois ou } A \text{ est realise}}{N}$$
...
Ceci represente la probabilite que $A$ se realise, sachant que $B$ s'est realise.
...
**Forme generalisee** (avec partition $A_1, \ldots, A_n$) :

$$P(A_i \mid B) = \frac{P(B \mid A_i) \cdot P(A_i)}{\sum_{j=1}^{n} P(A_j) \cdot P(B \mid A_j)}$$

| Terme | Nom | Interpretation |
|------|------|----------------|
| $P(A)$ | Prior (a priori) | Croyance sur $A$ avant d'observer $B$ |
| $P(A \mid B)$ | Posterieur (a posteriori) | Croyance mise a jour apres observation de $B$ |
