...
```r
c(1, 3, 5, 7, 9)            # Explicite : c() combine des elements
seq(0, 10, by=2)             # Sequence : 0, 2, 4, 6, 8, 10
0:10                         # Plage : 0, 1, 2, ..., 10 (inclusif !)
rep(1:2, 5)                  # Repeter : 1, 2, 1, 2, 1, 2, 1, 2, 1, 2
```

**IMPORTANT** : R utilise l'indexation a partir de 1, et les plages sont inclusives aux deux bornes. `1:10` inclut a la fois 1 et 10.
...
**Eviter les boucles quand c'est possible** -- utiliser les operations vectorisees pour la vitesse et la clarte.
...
df[1, ]              # Premiere ligne
df[, "height"]       # Colonne par nom en chaine
...
# tapply : operations groupees (LA plus utile)
...
replicate(1000, mean(rnorm(20)))  # 1000 moyennes echantillonnales de taille 20
```
...
# Schema de simulation : repeter l'experience de nombreuses fois
n_sim <- 10000
...
|---------|------------------|
| R divise par $n-1$ dans `var()`/`sd()` | C'est l'estimateur non biaise $S'^2$ ; si vous avez besoin de $S^2$, multiplier par $(n-1)/n$ |
| `read.csv()` utilise la virgule comme separateur | Utiliser `read.csv2()` pour les points-virgules (format europeen) |
| `hist()` affiche les effectifs par defaut | Utiliser `freq=FALSE` pour la densite (necessaire pour superposer des courbes theoriques) |
| Oublier `add=TRUE` dans `curve()` | Sans cela, un nouveau graphique est cree au lieu de superposer |
| `pnorm(x)` donne $P(X \leq x)$ | Pour $P(X > x)$, utiliser `1 - pnorm(x)` ou `pnorm(x, lower.tail=FALSE)` |
| `sample()` par defaut `replace=FALSE` | Specifier `replace=TRUE` pour l'echantillonnage avec remise |
...
| Operation groupee | `tapply(values, groups, function)` |
