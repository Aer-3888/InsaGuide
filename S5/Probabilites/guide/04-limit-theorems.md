# Chapitre 4 : Theoremes limites

## 4.1 Contexte et cadre

Considerons un processus ou l'on repete une experience de nombreuses fois :

- Chaque realisation est un tirage d'une variable aleatoire $X_i$
- Les $X_i$ sont **iid** (independantes et identiquement distribuees) avec $E[X_i] = \mu$, $Var(X_i) = \sigma^2$

La **moyenne empirique** est :

$$\bar{X}_n = \frac{1}{n}\sum_{i=1}^{n} X_i$$

Proprietes cles de $\bar{X}_n$ :

$$E[\bar{X}_n] = \mu \qquad Var(\bar{X}_n) = \frac{\sigma^2}{n}$$

**Intuition** : Moyenner reduit la variabilite. La moyenne de $\bar{X}_n$ egale la moyenne de $X$, mais sa variance decroit en $1/n$.

---

## 4.2 Inegalite de Tchebychev

Pour **toute** distribution ayant une moyenne et une variance finies :

$$P(|X - E[X]| \geq a) \leq \frac{Var(X)}{a^2}$$

De maniere equivalente, en termes d'ecarts-types :

$$P(|X - E[X]| \lt k\sigma) > 1 - \frac{1}{k^2}$$

| $k$ (ecarts-types) | Borne inferieure sur $P(\|X - \mu\| \lt k\sigma)$ |
|---|---|
| 2 | $\geq 75\%$ |
| 3 | $\geq 89\%$ |
| 4 | $\geq 93.75\%$ |

**Interpretation** : Pour toute distribution, au moins 75% des valeurs se trouvent a moins de 2 ecarts-types de la moyenne. C'est une borne universelle, mais souvent tres lache.

### Application a $\bar{X}_n$

En appliquant Tchebychev a la moyenne empirique :

$$P(|\bar{X}_n - \mu| > k\sigma) \leq \frac{1}{nk^2}$$

La borne diminue quand $n$ augmente -- c'est le fondement de la LGN.

### Exemple resolu

> Pour $X \sim \mathcal{E}(1)$ ($\mu = 1$, $\sigma = 1$), calculer $P(|X - 1| \lt k)$ exacte pour $k = 1, 2, 3$.

$$P(|X - 1| \lt k) = P(1-k \lt X \lt 1+k)$$

Comme la densite exponentielle est nulle pour $x \lt 0$ :

$$P(|X - 1| \lt k) = \int_0^{1+k} e^{-x}\, dx = 1 - e^{-(1+k)}$$

| $k$ | Borne Tchebychev | Valeur exacte |
|---|---|---|
| 1 | $\geq 0\%$ | 86% |
| 2 | $\geq 75\%$ | 95% |
| 3 | $\geq 89\%$ | 98% |

Les valeurs exactes sont bien meilleures que la borne de Tchebychev, illustrant que la borne est universelle mais conservative.

---

## 4.3 Loi des grands nombres (LGN)

### Enonce

Soient $X_1, X_2, \ldots, X_n$ iid avec $E[X_i] = \mu$ et $Var(X_i) = \sigma^2 \lt \infty$. Alors :

$$\forall \varepsilon > 0, \quad \lim_{n \to \infty} P(|\bar{X}_n - \mu| > \varepsilon) = 0$$

**En termes simples** : La moyenne empirique converge vers la moyenne theorique quand $n \to \infty$.

### Esquisse de preuve (via Tchebychev)

$$P(|\bar{X}_n - \mu| > \varepsilon) \leq \frac{Var(\bar{X}_n)}{\varepsilon^2} = \frac{\sigma^2}{n\varepsilon^2} \xrightarrow{n \to \infty} 0$$

### Conditions

La LGN requiert que $E[X]$ soit **finie**. Contre-exemple : la loi de Cauchy n'a pas de moyenne finie, et $\bar{X}_n$ ne converge PAS.

### Consequences pratiques

1. **Estimer des probabilites** : Utiliser la variable indicatrice $Y_i = \mathbf{1}_{X_i \in C}$. Alors $\bar{Y}_n \to P(X \in C)$ quand $n \to \infty$.
2. **Estimer des densites** : $f(a) \approx \bar{Y}_n / (2h)$ ou $Y_i = \mathbf{1}_{X_i \in (a-h, a+h)}$.
3. **Methodes de Monte Carlo** : Approcher des integrales par echantillonnage.

---

## 4.4 Theoreme central limite (TCL)

### Enonce

Soient $X_1, \ldots, X_n$ iid avec $E[X_i] = \mu$ et $Var(X_i) = \sigma^2$. Definissons la moyenne standardisee :

$$Z_n = \frac{\bar{X}_n - \mu}{\sigma / \sqrt{n}}$$

Alors quand $n \to \infty$ :

$$Z_n \xrightarrow{d} \mathcal{N}(0, 1)$$

De maniere equivalente :

$$\bar{X}_n \underset{n \to \infty}{\sim} \mathcal{N}\left(\mu, \frac{\sigma}{\sqrt{n}}\right)$$

### Intuition cle

Le TCL nous dit **la forme de la distribution** de $\bar{X}_n$ -- elle est approximativement normale, quelle que soit la distribution originale de $X$.

| Theoreme | Ce qu'il donne |
|---------|---------------|
| LGN | Une **valeur** (la moyenne) vers laquelle $\bar{X}_n$ converge |
| TCL | Une **distribution** ($\mathcal{N}$) qui decrit les fluctuations de $\bar{X}_n$ autour de $\mu$ |

### Intuition visuelle

Pour toute distribution de depart (gamma, bimodale, uniforme, etc.) :
- $\bar{X}_1 = X_1$ suit la distribution originale
- $\bar{X}_5$ commence a avoir une forme de cloche
- $\bar{X}_{30}$ est tres proche de la normale
- L'etalement diminue comme $\sigma / \sqrt{n}$

### Centrage-reduction

Pour comparer des variables d'echelles differentes, on standardise :

$$Z_n = \frac{\bar{X}_n - \mu}{\sigma / \sqrt{n}}$$

Verification : $E[Z_n] = 0$, $Var(Z_n) = 1$.

---

## 4.5 Exemples resolus

### Exemple 1 : Loi Gamma

> 500 tirages de $\Gamma(2, 1)$ ($\mu = 2$, $\sigma^2 = 2$). Apres $n = 400$ tirages : $\bar{X}_{400} = 1.99$. Apres $n = 500$ tirages : $\bar{X}_{500} = 2.06$. Est-ce que $\bar{X}_{500} = 2.06$ est inhabituel ?

Par le TCL : $\bar{X}_{500} \sim \mathcal{N}\left(2, \frac{\sqrt{2}}{\sqrt{500}}\right)$.

$$P(\bar{X}_{500} \geq 2.06) = 1 - \text{pnorm}(2.06, \text{mean}=2, \text{sd}=\sqrt{2/500})$$

$$= 1 - \text{pnorm}(2.06, 2, 0.0632) \approx 0.171$$

17% de chance d'etre au-dessus de 2.06 -- pas particulierement inhabituel.

**Lecon** : Ne pas comparer des ecarts bruts sans tenir compte de $n$. L'ecart de 0.06 a $n=500$ est en fait plus "normal" que ce que l'ecart de 0.01 a $n=400$ suggere.

### Exemple 2 : Erreurs d'arrondi

> 100 transactions arrondies a l'entier le plus proche. Erreur $X_i \sim \mathcal{U}(-0.5, 0.5)$ avec $\mu = 0$, $\sigma^2 = 1/12$. Trouver $P(|Y| > 10)$ ou $Y = \sum_{i=1}^{100} X_i$.

Etape 1 : $\bar{X}_{100} \sim \mathcal{N}(0, \sqrt{1/1200})$ par le TCL.

Etape 2 : $Y = 100\bar{X}_{100} \sim \mathcal{N}(0, \sqrt{100/12}) = \mathcal{N}(0, \sqrt{100 \cdot 1/12})$.

Etape 3 :
$$P(|Y| > 10) = P(Y \lt -10) + P(Y > 10)$$
$$= \text{pnorm}(-10, 0, \sqrt{100/12}) + 1 - \text{pnorm}(10, 0, \sqrt{100/12})$$
$$\approx 0.00053$$

---

## 4.6 Approximation normale de la binomiale

Lorsque $X \sim \mathcal{B}(n, p)$ avec $n$ grand :

$$X \approx \mathcal{N}(np, \sqrt{np(1-p)})$$

**Conditions** (regle empirique) : $n \geq 30$, $np \geq 5$, $n(1-p) \geq 5$.

### Correction de continuite

Comme la binomiale est discrete et la normale est continue :

| Exact (Binomiale) | Approximation (Normale) |
|---|---|
| $P(X = k)$ | $P(k - 0.5 \lt X \lt k + 0.5)$ |
| $P(X \geq k)$ | $P(X > k - 0.5)$ |
| $P(X \leq k)$ | $P(X \lt k + 0.5)$ |

---

## 4.7 Resume : LGN vs TCL

| | Loi des grands nombres | Theoreme central limite |
|---|---|---|
| **Donne** | Une valeur ($\mu$) | Une distribution ($\mathcal{N}$) |
| **Enonce** | $\bar{X}_n \to \mu$ | $\bar{X}_n \sim \mathcal{N}(\mu, \sigma/\sqrt{n})$ |
| **Requiert** | $E[X]$ finie | $E[X]$ et $Var(X)$ finies |
| **Utilite** | Justifie la prise de moyenne | Quantifie l'incertitude |

---

## AIDE-MEMOIRE -- Theoremes limites

| Resultat | Formule |
|--------|---------|
| $E[\bar{X}_n]$ | $\mu$ |
| $Var(\bar{X}_n)$ | $\sigma^2/n$ |
| Tchebychev | $P(\|X - \mu\| \geq a) \leq Var(X)/a^2$ |
| LGN | $P(\|\bar{X}_n - \mu\| > \varepsilon) \leq \sigma^2/(n\varepsilon^2) \to 0$ |
| TCL | $Z_n = (\bar{X}_n - \mu)/(\sigma/\sqrt{n}) \to \mathcal{N}(0,1)$ |
| Centrage-reduction | $Z = (X - \mu)/\sigma$ |
| Approx. binomiale | $\mathcal{B}(n,p) \approx \mathcal{N}(np, \sqrt{np(1-p)})$ lorsque $np\geq 5$, $n(1-p)\geq 5$ |
