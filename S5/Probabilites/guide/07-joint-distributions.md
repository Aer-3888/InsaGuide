# Chapitre 7 : Lois jointes et vecteurs aleatoires

## 7.1 Vecteurs aleatoires

Un **vecteur aleatoire** $\mathbf{X} = (X_1, X_2, \ldots, X_n)$ est un $n$-uplet de variables aleatoires definies sur le meme espace probabilise. Il associe a chaque resultat $\omega \in \Omega$ un element de $\mathbb{R}^n$.
...
- Valeurs RVB d'un pixel (dimensions mixtes)
...
La **PMF jointe** donne les probabilites pour toutes les combinaisons :

$$P(X_1 = x_1, X_2 = x_2, \ldots, X_n = x_n)$$

Pour le cas bivarie, cela peut s'ecrire sous forme de **tableau de probabilite** :
...
Verification : la somme de toutes les entrees vaut 1.
...
La **loi marginale** de $X$ est obtenue en "sommant" (ou integrant) sur les autres variables.
...
## 7.6 Vecteur moyen et matrice de covariance
...
Extension de la loi binomiale a $k > 2$ categories.

$n$ epreuves, chacune avec $k$ issues possibles de probabilites $(p_1, \ldots, p_k)$, $\sum p_i = 1$.
...
### Exemple resolu : Roulette

> 12 tours : Rouge (18/38), Noir (18/38), Vert (2/38). Quelle est $P(5\text{ rouge}, 5\text{ noir}, 2\text{ vert})$ ?
...
3. **Independance = decorrellation** : Pour les vecteurs gaussiens, les composantes sont independantes si et seulement si $\boldsymbol{\Sigma}$ est diagonale (non correlees).
4. **Projections scalaires** : $\mathbf{a}^T \mathbf{X} = \sum a_i X_i \sim \mathcal{N}(\sum a_i \mu_i, \sum a_i^2 \sigma_i^2)$ (si composantes independantes).

### TCL multivarie

Si $\mathbf{X}_1, \ldots, \mathbf{X}_N$ sont des vecteurs aleatoires iid dans $\mathbb{R}^n$ de moyenne $\mathbf{m}$ et covariance $\boldsymbol{\Sigma}$ :

$$\sqrt{N}(\bar{\mathbf{X}}_N - \mathbf{m}) \xrightarrow{d} \mathcal{N}(\mathbf{0}, \boldsymbol{\Sigma})$$

---

## 7.9 Exemple : loi normale bivariee
...
|---|---|
| Vecteur moyen | $\boldsymbol{\mu} = (E[X_1], \ldots, E[X_n])^T$ |
| Matrice de covariance | $\Sigma_{ij} = Cov(X_i, X_j)$ |
| Transformation lineaire (moyenne) | $E[\mathbf{AX}+\mathbf{b}] = \mathbf{A}\boldsymbol{\mu}+\mathbf{b}$ |
| Transformation lineaire (variance) | $Var(\mathbf{AX}+\mathbf{b}) = \mathbf{A}\boldsymbol{\Sigma}\mathbf{A}^T$ |
