...
Un intervalle de confiance (IC) remplace une estimation ponctuelle par un intervalle dont la probabilite de contenir la vraie valeur du parametre est connue.

**Exemple** : Au lieu de dire "le temps de reponse moyen est de 207.5 ms", on dit "nous sommes confiants a 95% que le temps de reponse moyen est entre 181 ms et 234 ms."
...
| Unilateral gauche | $[a, +\infty[$ | Garantir une valeur minimale |
| Unilateral droit | $]-\infty, b]$ | Garantir une valeur maximale |

---

## 5.2 Quantiles

Un **quantile d'ordre $p$** est la valeur $q$ telle que $P(X \lt q) = p$.

En R :
- `qnorm(p, mean, sd)` -- Quantile de la loi normale
- `qt(p, df)` -- Quantile de Student
- `qchisq(p, df)` -- Quantile du Chi-deux
- `qbinom(p, size, prob)` -- Quantile de la loi binomiale

**Exemple** : Pour $Z \sim \mathcal{N}(0,1)$ a l'IC 95% :
- $u_{0.025} = \text{qnorm}(0.025) \approx -1.96$
- $u_{0.975} = \text{qnorm}(0.975) \approx 1.96$
- Donc : $P(-1.96 \lt Z \lt 1.96) = 0.95$
...
Somme des carres de $k$ variables normales centrees reduites independantes :
...
Le rapport d'une normale centree reduite a la racine d'un chi-deux divise par ses degres de liberte :
...
La variance empirique **corrigee** (non biaisee) :

$$S'^2 = \frac{1}{n-1}\sum_{i=1}^n (X_i - \bar{X})^2 = \frac{n}{n-1}S^2 \qquad E[S'^2] = \sigma^2$$

**Important** : En R, `var()` et `sd()` calculent les versions non biaisees (division par $n-1$).

**Exemple numerique** : Pour $x = (2, 4, 6)$, $\bar{x} = 4$ :
- Biaisee : $S^2 = ((2-4)^2 + (4-4)^2 + (6-4)^2)/3 = 8/3 \approx 2.67$
- Corrigee : $S'^2 = ((2-4)^2 + (4-4)^2 + (6-4)^2)/2 = 4$ (c'est ce que R renvoie)
...
> Temps d'execution d'un programme sur 4 jours : 79, 79, 80, 82. $\sigma = 1$ connu. Trouver l'IC a 95% pour la moyenne.

$\bar{x} = 80$, $n = 4$, $\sigma = 1$, $u_{0.975} = 1.96$.

$$\mu \in \left[80 - 1.96 \cdot \frac{1}{2},\ 80 + 1.96 \cdot \frac{1}{2}\right] = [79.02, 80.98]$$

### Determination de la taille d'echantillon pour une precision cible
...
**Exemple** : Precision de 5 minutes, $\sigma = 10$, confiance 95% :
...
> Memes donnees : 79, 79, 80, 82. Maintenant $\sigma$ inconnu. Trouver l'IC a 95%.
...
**Observation** : L'IC est plus large quand $\sigma$ est inconnu car :
...
La loi du chi-deux est **asymetrique**, donc l'IC utilise des quantiles differents pour les bornes superieure et inferieure :

$$\boxed{\sigma^2 \in \left[\frac{(n-1)S'^2}{\chi^2_{n-1}(1-\alpha/2)},\ \frac{(n-1)S'^2}{\chi^2_{n-1}(\alpha/2)}\right]}$$

**Attention a l'inversion** : La borne inferieure de l'IC utilise le quantile **superieur**, et inversement.
...
En utilisant l'approximation binomiale-normale ($np \geq 5$, $n(1-p) \geq 5$, $n \geq 30$) :

$$\hat{p} \approx \mathcal{N}\left(p, \sqrt{\frac{p(1-p)}{n}}\right)$$

Comme $p$ apparait dans les bornes (circulaire), on utilise la borne conservative $p(1-p) \leq 1/4$ :

$$\boxed{p \in \left[\hat{p} - u_{1-\alpha/2}\frac{1}{2\sqrt{n}},\ \hat{p} + u_{1-\alpha/2}\frac{1}{2\sqrt{n}}\right]}$$

### Exemple resolu : Ports USB

> 7.5% des machines n'ont pas d'USB. Echantillon de 100. Trouver l'IC a 90% pour le nombre sans USB.

$X \sim \mathcal{B}(100, 0.075)$. Par le TCL : $X \approx \mathcal{N}(7.5, \sqrt{100 \cdot 0.075 \cdot 0.925})$.

Avec la binomiale exacte : $[\text{qbinom}(0.05, 100, 0.075),\ \text{qbinom}(0.95, 100, 0.075)] = [3, 12]$.

Avec l'approximation normale : $[\text{qnorm}(0.05, 7.5, 2.63),\ \text{qnorm}(0.95, 7.5, 2.63)] \approx [3.2, 11.8]$.
...
**R calcule en non biaise** : `var()` renvoie $S'^2$, `sd()` renvoie $S'$.
