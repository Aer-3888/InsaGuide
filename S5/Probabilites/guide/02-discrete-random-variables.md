...
Pour $X$ discrete de valeurs $\{x_1, x_2, \ldots\}$ :

$$P(X = x_i) = p_i \quad \text{avec} \quad \sum_i p_i = 1$$

La PMF determine completement la distribution : la liste de toutes les valeurs possibles avec leurs probabilites.
...
**Application** : Essai unique a deux issues (erreur dans un modele, lancer de piece, detection de defaut).
...
## 2.4 Exemples resolus

### Exemple 1 : QCM au hasard

> Un QCM de 10 questions avec 4 choix chacune. Un etudiant repond au hasard. Quelle est $P(\text{reussir})$ si la reussite requiert $\geq 6$ bonnes reponses ?
...
Seulement environ 2% de chance de reussir en repondant au hasard.

### Exemple 2 : Probleme d'urne

> Une urne contient 8 boules rouges et 5 noires. On tire 6 boules sans remise. Nombre attendu de boules rouges ?
