...
Un **test d'hypothese** est une methode statistique permettant de prendre des decisions a partir de donnees -- il determine si les resultats observes sont **statistiquement significatifs** ou pourraient etre dus au hasard.
...
| $H_1$ (hypothese alternative) | Ce que l'on veut montrer / detecter |

**Convention** :
- **Ethique** : $H_1$ = ce que l'on espere demontrer
- **Pratique** : $H_0$ = un enonce bien defini et testable (par ex. $\mu = \mu_0$)
...
5. **Decider** : $s$ est-il dans la region d'acceptation (zone de forte probabilite) ou la region de rejet ?
...
Tester si un echantillon est coherent avec une valeur hypothetique du parametre.
...
### Exemple resolu : Pieces isolantes

> Un fournisseur affirme que l'epaisseur moyenne est de 7.3 mm. Echantillon de 24 pieces : $\bar{x} = 7.126$, $\sigma = 0.38$ (connu). Test au niveau $\alpha = 0.05$.

$$z = \frac{7.126 - 7.3}{0.38/\sqrt{24}} = \frac{-0.174}{0.0776} = -2.24$$

Region d'acceptation : $[-1.96, 1.96]$. Puisque $-2.24 \notin [-1.96, 1.96]$, **rejeter $H_0$**.

p-valeur : $2 \times \text{pnorm}(-2.24) = 0.025 \lt 0.05$.

Avec $\sigma$ inconnu ($s' = 0.395$) : $t = \frac{7.126 - 7.3}{0.395/\sqrt{24}} = -2.157$. La region d'acceptation pour $t_{23}$ au niveau $\alpha = 0.05$ est $[-2.069, 2.069]$. Puisque $|-2.157| > 2.069$, on **rejette $H_0$** egalement.
...
### Cas 2 : $\sigma_1 = \sigma_2$ inconnu (t-test groupe)

Estimer $\sigma$ par la variance groupee :
...
### Cas 3 : $\sigma_1 \neq \sigma_2$ inconnu (test de Welch / Aspin-Welch)
...
# z-test (sigma connu) -- necessite le package TeachingDemos
...
2. **Taille d'echantillon** $n$ : plus $n$ est grand, plus la puissance est elevee
3. **Niveau de signification** $\alpha$ : plus $\alpha$ est grand, plus la puissance est elevee (mais plus d'erreurs de type I)
4. **Variabilite** $\sigma$ : plus $\sigma$ est petit, plus la puissance est elevee
...
> Bouteilles de lait : $\sigma = 1$ ml, $\mu_0 = 1000$ ml, $n = 40$. Detecter un ecart de 0.2 ml.
...
1. **Histogramme vs courbe normale** : comparaison visuelle en superposition
2. **Boite a moustaches** : verifier la symetrie (mediane centree, moustaches egales)
3. **QQ-plot** : graphique quantile-quantile contre la normale theorique ; les points doivent s'aligner sur une droite

### Tests formels

| Test | Fonction R | Adapte pour |
|------|-----------|----------|
| Shapiro-Wilk | `shapiro.test(x)` | $n \leq 50$ |
| Kolmogorov-Smirnov | `ks.test(x, "pnorm", mean, sd)` | Grands echantillons |

Les deux testent $H_0$ : les donnees sont normalement distribuees. Rejeter si p-valeur $\lt \alpha$.

---

## 6.8 Liste de verification methodologique
...
5. Determiner la region d'acceptation ou la p-valeur
...
**Regle de la p-valeur** : Rejeter $H_0$ si p-valeur $\lt \alpha$.
