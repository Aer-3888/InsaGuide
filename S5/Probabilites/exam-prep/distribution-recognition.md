...
> Pendant l'examen, vous devez rapidement identifier quelle distribution s'applique a un probleme. Ce guide fournit des regles de correspondance.
...
|---|---|---|
| "Succes/echec", "oui/non", epreuve unique | Bernoulli $\mathcal{B}(p)$ | $p$ = prob. de succes |
| "$n$ epreuves independantes, compter les succes" | Binomiale $\mathcal{B}(n,p)$ | $n$ epreuves, $p$ prob. succes |
| "$k$ evenements dans un temps/espace avec taux moyen $\lambda$" | Poisson $\mathcal{P}(\lambda)$ | $\lambda$ = nombre moyen |
| "Nombre d'epreuves jusqu'au premier succes" | Geometrique $\mathcal{G}(p)$ | $p$ = prob. succes |
...
| "Rapport : normale / racine(chi-deux/ddl)" | Student $t_k$ | $k$ degres de liberte |
...
Estimer une PROPORTION :
  +-- n grand (np>=5, n(1-p)>=5) --> N(0,1) : utiliser qnorm()
  +-- n petit --> Binomiale exacte : utiliser qbinom()
```
...
## Reconnaissance des schemas de problemes d'examen

### Schema 1 : "Donnees brutes, trouver l'IC pour la moyenne"

**Approche** :
1. Calculer $\bar{x}$ et $s'^2$ (ou $s'$)
2. $\sigma$ est-il donne ? (generalement NON a l'examen)
3. Utiliser Student $t_{n-1}$
...
### Schema 2 : "Statistiques resumees ($\sum x_i$, $\sum x_i^2$), trouver l'IC"

**Approche** :
1. $\bar{x} = \sum x_i / n$
2. $s'^2 = \frac{1}{n-1}(\sum x_i^2 - n\bar{x}^2)$
3. Continuer comme le Schema 1
...
### Schema 3 : "Comparer deux groupes"

**Approche** :
1. Identifier : test d'homogeneite ($H_0: \mu_1 = \mu_2$)
2. Les variances sont-elles connues ? Egales ? Differentes ?
3. Selectionner le test approprie (voir Ch.6)
4. Calculer $D = \bar{x}_1 - \bar{x}_2$ et $\hat{\sigma}_D$
5. Comparer $T = D/\hat{\sigma}_D$ a la valeur critique

### Schema 4 : "Cette valeur est-elle coherente avec les donnees ?"

**Approche** :
1. Identifier : test de conformite ($H_0: \mu = \mu_0$)
2. Verifier si $\mu_0$ tombe dans l'IC du Schema 1
3. OU calculer la statistique de test et comparer a la region critique

### Schema 5 : "Multinomiale/vote/categories"

**Approche** :
1. Identifier la variable comme multinomiale
2. Utiliser `dmultinom()` pour les probabilites specifiques
3. Pour les proportions, possibilite d'approximation normale pour l'IC

### Schema 6 : "Trouver la variance d'une combinaison lineaire"

**Approche** :
1. Ecrire $Y = \mathbf{a}^T\mathbf{X}$
2. $Var(Y) = \mathbf{a}^T\boldsymbol{\Sigma}\mathbf{a}$
3. Developper : $Var(\sum a_i X_i) = \sum a_i^2 Var(X_i) + 2\sum_{i \lt j} a_i a_j Cov(X_i, X_j)$
...
|---|---|---|
| $u_{0.975}$ | 1.960 | IC 95%, $\sigma$ connu |
| $u_{0.995}$ | 2.576 | IC 99%, $\sigma$ connu |
| $u_{0.95}$ | 1.645 | IC 90% ou unilateral 95% |
...
- Les bornes de l'IC pour la variance doivent etre positives
- La statistique t doit etre negative si $\bar{x} \lt \mu_0$, positive si $\bar{x} > \mu_0$
- La p-valeur doit etre entre 0 et 1
- Si p-valeur $\lt \alpha$, alors $\mu_0$ ne devrait PAS etre dans l'IC (et inversement)
