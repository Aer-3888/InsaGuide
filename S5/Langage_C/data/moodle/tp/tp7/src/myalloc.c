...
void Mon_free(void* ptr){
  ...
  /* I consider that a free on null is valid */
  if (ptr == NULL) {
	  debug_print("Mon_free() : Pointer is null, thanks.");
	  return; /* Nothing to do */
  }
  /* Does it point to a pointer on the heap ? */
  if (!est_dans_tas(ptr)) {
	  debug_print("Mon_free() : Pointer not on heap. Abort.");
	  return; /* should be an error here but oh well */
  }

  /* draw the rest of the owl */
  /* Get the memory descriptor pointer */
  descriptor = ptr_descripteur(ptr);
  if (descriptor == NULL) {
	  debug_print("Mon_free() : Could not get descriptor. Abort.");
	  return; /* Could not get descriptor */
  }

  /* Maybe the area is already free */
  if (statut_zone(descriptor, LIBRE)) {
	  debug_print("Mon_free() : Area to free is already free. Abort.");
	  return; /* It is. For security's sake, do nothing */
  }
  /* Otherwise, free the area */
...
void *Mon_calloc(size_t count, size_t size) {
  ...
  /* Check if count or size is null */
  if (count * size == 0) {
...
void *Mon_realloc(void *ptr, size_t newsize) {
  ...
  /* votre code ici */
  /* If the pointer is null, simply malloc */
  if (ptr == NULL) {
	  debug_print("Mon_realloc() : ptr is null, doing malloc");
	  return Mon_malloc(newsize);
  }

  /* if size is null and ptr isn't, free */
  if (newsize == 0 && ptr != NULL) {
	  debug_print("Mon_realloc() : size is null and ptr isn't. Free.");
	  Mon_free(ptr);
	  return NULL;
  }

  /* Check that the pointer is valid on the heap */
  if (!est_dans_tas(ptr)) {
  ...
  /* If the area was marked as free… don't touch anything */
  if (statut_zone(descriptor, LIBRE)) {
  ...
  if (newsize < descriptor->size) {
	  ...
	  /* Do some housekeeping in case we can merge free areas */
	  regroupement_libre();

	  /* The final pointer is resultat */
	  resultat = ptr;
  } else {
	  /* Increase the block by :
	   * Checking if the next block is free and big enough
	   * or
	   * Allocating a new block, moving stuff, freeing this one
	   */
	  debug_print("Mon_realloc() : extending…");

	  if (
		/* if there is no next block */
		descriptor->suiv == NULL
		/* or if it's occupied */
		|| descriptor->suiv->statut == OCCUPE
		/* or if it's not big enough */
		|| descriptor->suiv->size < (newsize-descriptor->size)) {
		  debug_print("Mon_realloc() : …by new allocation");
		  resultat = Mon_malloc(newsize);
		  if (resultat == NULL) {
			  debug_print("Mon_realloc() : Underlying malloc failed. Abort.");
			  return NULL;
		  }
		  
		  /* move previous data */
		  for (scan_index = 0; scan_index < descriptor->size; scan_index++)
			  *(((int*)resultat)+scan_index) = *(((int*)ptr)+scan_index);

		  /* free the old one */
		  free(ptr);

		  /* Merge free areas if needed */
		  regroupement_libre();
	  } else {
		  debug_print("Mon_realoc() : …by merging");

		  /* Not exactly thread safe but oh well */
		  descriptor->statut = LIBRE;
		  ...
		  /* There's no size check here because if we
		   * failed we're screwed anyways
		   */
