/*
 * TP3 - Tests pour les predicats de manipulation de listes
 *
 * Utilisation : [listes].
 *               [tp_listes_tests].
 *               tests.
...
test_membre :-
    assert_true( membre(a, [a,b,c]) ),
    assert_true( not(membre(0, [1,2,3])) ),
    assert_true( sortall(A, membre(A,[2,1,3,1]), [1,1,2,3]) ).

test_compte :-
    assert_true( compte(a, [a,c,a,b,a,c,b], 3) ),
    assert_true( findall(N, compte(b, [a,c,a,b,a,c,b], N), [2]) ).

test_comptefinal :-
    assert_true( comptefinal(a, [a,c,a,b,a,c,b], 3) ),
    assert_true( sortall(A, comptefinal(A, [a,c,a,b,a,c,b], 2), [b,c]) ),
    assert_true( sortall([A,N], comptefinal(A,[b,c,a,b,a,a],N), [[a,3],[b,2],[c,1]]) ).

test_renverser :-
    assert_true( renverser([a,b,c], [c,b,a]) ),
    assert_true( findall(Y, renverser([1,2,2,4], Y), [[4,2,2,1]]) ).
...
test_sous_liste :-
    ...
    assert_true( setof(L, sous_liste([1,2,3],L), [[],[1],[1,2],[1,2,3],[2],[2,3],[3]]) ).
...
test_inclus2 :-
    assert_true( inclus2([3,2], [1,2,3,4]) ),
    assert_true( not(inclus2([3,55], [1,2,3,4])) ),
    assert_true( sortall(A, inclus2(A, [1,2]), [[], [1], [1, 2], [2], [2, 1]] ) ).
