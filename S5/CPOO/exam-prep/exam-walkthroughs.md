...
See the worked example in [04-uml-diagrams.md](../guide/04-uml-diagrams.md). Key: identify the Composite pattern where `Noeud` (node) is either a `Valeur`, `RefConstante`, or `Operateur` (with `Addition` and `Soustraction` as subtypes).
