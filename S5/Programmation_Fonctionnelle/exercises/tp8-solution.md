# TP8 - Jeu de cartes (variante Graphics)

> D'apres les instructions du TP : `data/moodle/tp/tp8/README.md`

Similaire au TP3 mais avec le variant tuple `Carte of haut * coul` au lieu du record `{h: haut; c: coul}`.
...
type carte = Carte of haut * coul    (* variant tuple au lieu de record *)
```
...
### Construction de cartes avec variants tuples
...
### Logique du jeu de reussite avec pattern matching sur tuples de cartes

`string_of_carte` fait du pattern matching directement sur `Carte (h, col)`. La generation aleatoire utilise `List.mem` au lieu de `exist` ecrit a la main.

**Reponse :**
```ocaml
(* Afficher une carte par pattern matching sur Carte *)
...
**Reponse :**
```ocaml
(* Une etape de reduction : pattern matching sur Carte (ah, ac) directement *)
...
(* Appliquer les reductions jusqu'a stabilite (comparaison par longueur de liste) *)
let rec reussite l =
  let newl = reduc l in
...
- `draw_pile` : dessine une pile verticale de cartes
