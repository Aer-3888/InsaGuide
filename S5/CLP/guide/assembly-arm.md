...
| r12 | ip | Registre temporaire intra-procedure |
...
.bss                     @ Section de donnees non initialisees
    result: .skip 4      @ Reserver 4 octets (1 mot)
    buffer: .space 100   @ Reserver 100 octets
...
| `.align` | remplissage jusqu'a la frontiere de mot | `.align` |

### Constantes

```arm
.equ NAME, value        @ Constante a l'assemblage
...
LDR  rd, [rn]           @ rd = memoire[rn]  (charger mot)
LDR  rd, [rn, #offset]  @ rd = memoire[rn + offset]
LDR  rd, =label         @ rd = adresse du label (pseudo-instruction)
STR  rd, [rn]           @ memoire[rn] = rd  (stocker mot)
STR  rd, [rn, #offset]  @ memoire[rn + offset] = rd
LDRB rd, [rn]           @ rd = memoire[rn] (charger octet, extension zero)
STRB rd, [rn]           @ memoire[rn] = rd (stocker octet)
```
...
str r1, [r0]            @ memoire[adresse de x] = r1
```
...
```arm
@ Adressage avec offset
ldr r0, [r1, #4]        @ r0 = memoire[r1 + 4]    (pre-indexe)
ldr r0, [r1, r2]        @ r0 = memoire[r1 + r2]
ldr r0, [r1, r2, lsl #2] @ r0 = memoire[r1 + r2*4] (index a echelle)

@ Adressage post-indexe
ldr r0, [r1], #4        @ r0 = memoire[r1], puis r1 = r1 + 4

@ Pre-indexe avec mise a jour
ldr r0, [r1, #4]!       @ r1 = r1 + 4, puis r0 = memoire[r1]
```
...
MOV  rd, rm, LSR #n     @ rd = rm >> n   (decalage logique a droite, remplir avec 0)
MOV  rd, rm, ASR #n     @ rd = rm >> n   (decalage arithmetique a droite, extension de signe)
MOV  rd, rm, ROR #n     @ rd = rm en rotation droite de n
```
...
BX   lr                 @ Branchement a l'adresse dans lr (retour de fonction)
BEQ  label              @ Brancher si Z=1 (egal)
BNE  label              @ Brancher si Z=0 (different)
BGT  label              @ Brancher si N=V et Z=0 (signe superieur)
BLT  label              @ Brancher si N!=V (signe inferieur)
BGE  label              @ Brancher si N=V (signe superieur ou egal)
BLE  label              @ Brancher si Z=1 ou N!=V (signe inferieur ou egal)
BHI  label              @ Brancher si C=1 et Z=0 (non-signe superieur strict)
BHS  label              @ Brancher si C=1 (non-signe superieur ou egal)
BLO  label              @ Brancher si C=0 (non-signe inferieur)
...
```arm
ADDEQ r0, r0, #1       @ Ajouter 1 a r0 SEULEMENT SI le drapeau Zero est mis
MOVGT r1, r0            @ Deplacer r0 vers r1 SEULEMENT SI superieur
STRNE r2, [r3]          @ Stocker SEULEMENT SI different
```

Cela evite les branchements courts et est une caracteristique signature de l'ISA ARM.

**Exemple** (du TD assembleur, comptage dans un tableau) :
```arm
cmp r0, #0
addlt r3, r3, #1       @ Si r0 < 0 : incrementer compteur negatifs
addeq r4, r4, #1       @ Si r0 == 0 : incrementer compteur zeros
```
...
### Pile descendante pleine (Full Descending)

ARM utilise par convention une pile descendante pleine (FD) :
- **Pleine (Full)** : SP pointe vers le dernier emplacement occupe
...
STMFD sp!, {r0, r1, r2}    @ Empiler r0, r1, r2 sur la pile
                            ...
                            @ Registres stockes : r2 a l'adresse haute, r0 a la basse

LDMFD sp!, {r0, r1, r2}    @ Depiler de la pile dans r0, r1, r2
                            @ SP augmente de 12
```

**IMPORTANT** : STMFD stocke les registres en ordre croissant de numero de registre depuis l'adresse basse. Donc `STMFD sp!, {r0, r1}` stocke r0 a [sp-8] et r1 a [sp-4].
...
## 6.6 Convention d'appel de fonctions
...
Adresses hautes
    ...
    | Argument 1       |  [fp, #8]
    +------------------+
    | Valeur de retour |  [fp, #8] (si resultat passe via pile)
    +------------------+
    | LR sauvegarde    |  [fp, #4]
    +------------------+
...
@ 3. Reserver l'espace pour la valeur de retour (si retour par pile)
sub sp, sp, #4
...
### Responsabilites de l'appele (Prologue/Epilogue)
...
### Patron pour les fonctions recursives

```arm
recursive_func:
    @ Prologue : DOIT sauvegarder LR car BL va l'ecraser
    stmfd sp!, {lr}
    stmfd sp!, {fp}
    mov fp, sp
...
pgcd:
    ...
    cmp r0, #0
    beq retZ                 @ a == 0 : retourner 0
    cmp r1, #0
    beq retZ                 @ b == 0 : retourner 0
    cmp r0, r1
    beq retA                 @ a == b : retourner a
    bgt retP1                @ a > b : recurser(a-b, b)
...
```arm
@ Parcourir le tableau, chargeant chaque element
...
.equ offset_count, 4        @ Second champ : int count
...
```arm
ldr r2, [r0, r3, lsl #2]    @ r2 = array[r3] (pointeur vers struct)
...
### Exemple 1 : Compter des caracteres (Annales 2017 Exercice 1)
...
**Techniques cles** : Adressage post-increment, execution conditionnelle (`addeq`), parcours de chaine terminee par null. Notez que le "e" accentue dans "litterature" ne correspond pas au 'e' ASCII, donc le compteur final est 5.
...
collatz_loop:
  cmp r0, #1
  beq collatz_done           @ s'arreter quand x == 1
...
collatz_store:
  str r0, [r2]               @ sauvegarder x en memoire
  b collatz_loop
collatz_done:
```

**Techniques cles** : Test de bits avec AND, MLA pour 3x+1, LSR pour division par 2.

### Exemple 3 : Factorielle (du TD)

```arm
.set N, 12                   @ Maximum de factorielle tenant dans 32 bits
.data
  i:    .word 1
  fact: .word 1
.text
...
**Note** : 12! = 479001600 tient dans 32 bits. 13! = 6227020800 ne tient PAS.
...
Sur Raspberry Pi, les registres materiels sont accedes en lisant/ecrivant a des adresses memoire specifiques. Aucune instruction d'E/S speciale n'est necessaire.

```arm
.set GPSEL4, 0x3f200010     @ Registre de selection de fonction GPIO 4
.set GPSET1, 0x3f200020     @ Registre de mise a 1 GPIO
.set GPCLR1, 0x3f20002c     @ Registre de mise a 0 GPIO
```

### Configurer une broche en sortie

La GPIO 47 utilise les bits [21:19] de GPFSEL4. Mettre a 001 = mode sortie :
...
### Considerations bare-metal

- Pas d'OS, pas d'initialisation de pile (utiliser les registres pour sauvegarder/restaurer)
- Pas de printf -- observer les LED ou utiliser un debugger materiel
- Edition de lien a 0x8000 (`-Ttext=0x8000`) -- adresse de demarrage du Raspberry Pi
- Boucles d'attente active pour le timing (pas de configuration d'interruption timer)

---

## 6.11 Pieges courants

1. **Oublier de sauvegarder LR** : Si votre fonction appelle une autre fonction (BL), LR est ecrase. Toujours faire STMFD {lr} avant un BL si vous devez retourner.

2. **Desalignement de pile** : Chaque STMFD doit avoir un LDMFD correspondant. Chaque SUB sp doit avoir un ADD sp correspondant. Des paires non assorties corrompent la pile.

3. **Mauvais calculs d'offset** : Dessiner le cadre de pile et compter les octets. Rappel : LR est a fp+4, FP a fp+0, valeur de retour a fp+8, premier parametre a fp+12 (en convention standard).

4. **LDRB vs LDR** : Utiliser LDRB pour les caracteres/octets individuels, LDR pour les mots de 32 bits. Utiliser LDR sur un tableau d'octets lit 4 octets d'un coup, donnant des valeurs erronees.

5. **Indexation a echelle** : Pour les tableaux de mots, utiliser `lsl #2` pour multiplier l'index par 4. Pour les tableaux d'octets, pas de mise a l'echelle. Oublier la mise a l'echelle lit les mauvais elements.

6. **Piege de l'execution conditionnelle** : `ADDEQ` ne fait rien si le drapeau Z n'est pas mis. S'assurer que CMP est execute immediatement avant l'instruction conditionnelle -- d'autres instructions entre CMP et le conditionnel peuvent modifier les drapeaux.
...
8. **Restriction de destination de MUL** : Sur certaines variantes ARM, `MUL rd, rm, rs` exige rd != rm. Utiliser un registre different comme destination en cas de doute.
...
```
REGISTRES :
  r0-r3 :  Arguments / Retour / Temporaires (sauvegardes par l'appelant)
  r4-r10 : Usage general (sauvegardes par l'appele)
  r11/fp : Pointeur de cadre (Frame Pointer)
...
OFFSETS DU CADRE DE PILE :
  [fp, #0]  = FP sauvegarde
  [fp, #4]  = LR sauvegarde
  [fp, #8]  = valeur de retour (ou 1er param)
  [fp, #12] = 1er param (ou 2e param)
...
INSTRUCTIONS CLES :
  ...
  RSB rd, rn, op2       @ rd = op2 - rn (soustraction inverse)
  LSL #n                @ decalage gauche de n (multiplier par 2^n)
  LSR #n                @ decalage droite de n (diviser par 2^n)
```
