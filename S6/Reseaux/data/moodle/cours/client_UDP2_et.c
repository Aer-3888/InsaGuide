...
int main(int argc, char** argv) {
  ...
  serveur.sin_port = htons(......);
#ifdef WIN32
    serveur.sin_addr.s_addr =inet_addr(........);
#else
    inet_aton(........., (struct in_addr *)&serveur.sin_addr);
#endif
  ...
  while (nb_question < NB_REQUETES) {
    ...
    printf("client %2s: (%s,%4d) recoit de ", id
	   , inet_ntoa(moi.sin_addr),ntohs(moi.sin_port));
    ret = recvfrom(.........,.......,.........,0, (struct sockaddr *)
		   , );
    if (ret <= 0) {
