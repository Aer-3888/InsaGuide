# TP2 - Construction d'un pipeline ML : rendez-vous manques

> Instructions du TP issues de : `data/moodle/tp/tp2_ml_pipeline/TP2_complete.ipynb`

---

## Exercice 1 : Description des donnees

### Charger le dataset et examiner sa structure.
...
Le dataset contient 14 colonnes avec les attributs suivants :
...
| ScheduledDay | Datetime | Date et heure de la prise de rendez-vous |
| AppointmentDay | Datetime | Date du rendez-vous prevu |
...
| Hipertension | Binaire | 1 si l'individu a de l'hypertension |
| Diabetes | Binaire | 1 si l'individu a du diabete |
| Alcoholism | Binaire | 1 si l'individu souffre d'alcoolisme |
| Handcap | Binaire | 1 si l'individu est handicape |
| SMS_received | Binaire | 1 si l'individu a recu un SMS de rappel |
...
# Aussi diviser par 86400 pour convertir les secondes en jours
...
**Explication :** La feature `AppointmentDelay` est creee en calculant la difference entre la date du rendez-vous et la date de prise de rendez-vous, en convertissant en jours, et en plafonnant les valeurs negatives a 0. C'est l'etape d'ingenierie de features cle de ce TP -- c'est la feature la plus importante au final.

---

## Exercice 3 : Separation des jeux d'entrainement et de test
...
# Retirer PatientId et AppointmentID : identifiants, pas des features
...
**Explication :** Le meme split train/test est reutilise pour TOUS les modeles de ce TP afin de garantir une comparaison equitable. PatientId et AppointmentID sont retires car ce sont des identifiants, pas des features predictives.

---

## Exercice 4 : Random Forest (benchmark)

### Utiliser un Random Forest comme benchmark avec validation croisee, puis analyser l'importance des features.
...
the_rand_forest = RandomForestClassifier()  # Par defaut : 100 arbres
before = time.time()
...
**Explication :** La feature construite `AppointmentDelay` (26.32%) est la plus importante. Plus le delai entre la prise de rendez-vous et la date du rendez-vous est long, plus il est probable que le patient ne se presente pas. Les features medicales (Hipertension, Diabetes, etc.) contribuent chacune a moins de 1% -- ce sont de mauvais predicteurs des absences.
...
# Obtenir un echantillon aleatoire de 5% pour la rapidite de visualisation
...
**Sortie attendue :** Une matrice de nuages de points croises pour les features les plus importantes, colores par classe (present vs absent). Un clustering notable apparait dans `AppointmentDay/AppointmentDelay` et `AppointmentDelay/Age`.

---

## Exercice 5 : K plus proches voisins (KNN)
...
**Sortie attendue :** Les points "Show" (vert) et "No-show" (rouge) sont fortement melanges. Il n'y a pas de frontiere de decision claire en 2D, confirmant la difficulte de ce dataset.

### 5c : Tuning du meta-parametre K
...
**Sortie attendue :** La courbe de validation monte rapidement de K=1 (~72%) a K=20 (~79%), puis forme un plateau autour de 79.7-79.8% pour K entre 50 et 200. La courbe est lisse et bien definie grace au split fit/validation fixe.
...
**Explication :** KNN avec K=60 surpasse legerement le benchmark Random Forest. La courbe de validation a un plateau clair, indiquant une performance robuste. Initialement, avec des splits train/validation changeants a chaque fit, les courbes etaient tres bruitees -- utiliser un split fixe produit des resultats beaucoup plus propres.

---

## Exercice 6 : Classification Naive Bayes
...
### 6b : Approche 1 -- Suppression des variables numeriques

Conserver uniquement les 8 features categorielles/binaires et utiliser `CategoricalNB`.
...
**Explication :** Malgre la perte des features les plus importantes (AppointmentDelay, ScheduledDay, Age), CategoricalNB atteint 79.76% -- presque identique a KNN. Cela suggere que les features categorielles/binaires portent suffisamment d'information pour ce probleme.

### 6c : Approche 2 -- Discretisation des features continues en categories
...
| 0-12 | enfant | 0 |
| 13-19 | adolescent | 1 |
| 20-30 | jeune adulte | 2 |
| 31-50 | adulte | 3 |
| 51-65 | senior | 4 |
| 66-80 | personne agee | 5 |
| 81+ | grand age | 6 |
...
**Explication :** La discretisation permet d'utiliser toutes les features avec CategoricalNB, mais le score (79.08%) est legerement inferieur a l'Approche 1 (79.76%). La perte d'information due au regroupement des features continues en categories grossieres n'est pas compensee par le gain d'avoir plus de features.

---

## Exercice 7 : Boosting par stumps (AdaBoost)

### 7a : Tuning du nombre d'estimateurs
...
Le meilleur nombre d'estimateurs varie entre les executions (1, 7, 50, 100...).
...
**Sortie attendue :** Contrairement a la courbe KNN qui etait lisse, la courbe de validation AdaBoost est **chaotique et erratique**. Le score oscille entre 79% et 80% sans tendance claire. Il n'y a pas de nombre optimal evident d'estimateurs.
...
**Explication :** Le boosting n'offre aucun gain de performance significatif sur ce dataset. La courbe de validation est chaotique, ce qui signifie qu'il n'y a pas de valeur stable optimale pour le nombre d'estimateurs. L'accuracy ne depasse jamais de maniere consistante les 80%.
...
| Modele | Configuration | Accuracy test | Notes |
|--------|--------------|---------------|-------|
| Random Forest | 100 arbres, CV=4 | 78.06% (CV) / 79.8% (test) | Bon benchmark, importance des features |
| KNN | K=60 | 79.77% | Meilleur modele, plateau clair |
| Naive Bayes (categoriel) | CategoricalNB | 79.76% | Etonnamment bon malgre les features manquantes |
| Naive Bayes (discretise) | CategoricalNB + binning | 79.08% | La discretisation perd de l'information |
| AdaBoost | n_estimators variable | ~79-80% | Pas de gain significatif, courbe chaotique |

**Resultats cles :**

1. **L'ingenierie de features est essentielle.** La feature construite `AppointmentDelay` est le predicteur le plus important (26.32% d'importance dans Random Forest). La connaissance du domaine ameliore tous les modeles.

2. **Les modeles simples peuvent surpasser les modeles complexes.** KNN avec K=60 est le meilleur modele bien qu'etant l'une des approches les plus simples.

3. **Pourquoi aucun modele ne depasse 80%.** Le taux d'absence est d'environ 20%, ce qui signifie qu'un classifieur naif qui predit toujours "present" atteint deja ~80%. Tous les modeles battent a peine cette ligne de base, suggerant que les features disponibles sont faiblement predictives et que des facteurs importants (motivation personnelle, urgences, oubli) ne sont pas captures dans les donnees.

4. **Des splits train/test coherents sont importants.** Utiliser le meme split pour tous les modeles garantit une comparaison equitable. Utiliser le meme split fit/validation dans le tuning KNN produit des courbes de validation plus propres et reproductibles.
