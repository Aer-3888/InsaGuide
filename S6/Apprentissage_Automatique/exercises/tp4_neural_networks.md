...
> Instructions du TP issues de : `data/moodle/tp/tp4_neural_networks/tp_nn.py` et `data/moodle/tp/tp4_neural_networks/README.md`
...
| Dimensions | 32 x 32 pixels, 3 canaux (RGB) |
...
### Charger le dataset, normaliser les valeurs de pixels, et preparer les representations aplatie et spatiale.
...
# Normaliser les valeurs de pixels de [0, 255] a [0.0, 1.0]
...
**Explication :** Normaliser les pixels dans [0, 1] aide l'optimiseur (Adam) a converger plus vite, empeche la saturation des neurones et ameliore la stabilite numerique. Sans normalisation, l'entrainement est tres lent ou instable.

### Redimensionnement pour chaque type de modele
...
## Exercice 2 : Perceptron simple (Multi-Layer Perceptron)

### Implementer un perceptron avec une couche cachee, l'entrainer sur CIFAR-10 et l'evaluer.
...
| `Dense(units=10, activation='softmax')` | Couche de sortie avec 10 neurones (un par classe). Softmax convertit les 10 valeurs en probabilites sommant a 1. |
| `sparse_categorical_crossentropy` | Fonction de cout pour la classification multiclasse. "Sparse" signifie que les labels sont des entiers (0-9), pas des vecteurs one-hot. |
...
- `epochs=20` : le modele voit l'ensemble d'entrainement 20 fois.
...
**Explication de `argmax(-1)` :** Pour chaque image, le modele produit un vecteur de 10 probabilites. `argmax(-1)` retourne l'indice du neurone avec la probabilite la plus elevee, qui correspond au numero de classe predit (0=airplane, 1=automobile, ..., 9=truck).

**Resultats attendus pour differentes tailles de couche cachee :**

| nbhidden | Accuracy test | Parametres | Commentaire |
|----------|--------------|-----------|-------------|
| 100 | ~40-45% | ~308K | Minimum viable |
| 1 000 | ~48-52% | ~3M | Amelioration marginale |
| 10 000 | ~50-53% | ~30M | Rendements decroissants |
| 100 000 | ~50-55% | ~308M | Overfitting massif probable |
...
**Explication :** Meme avec 100 000 neurones caches (307 millions de parametres), le MLP ne peut pas depasser ~55% sur CIFAR-10. Le probleme n'est pas la capacite du modele mais l'architecture : en aplatissant l'image, toute la structure spatiale est perdue. Le pixel (0,0) est traite de maniere identique au pixel (31,31), meme s'ils n'ont aucune relation spatiale. L'ecart entre l'accuracy train (~92%) et test (~50%) indique un **overfitting massif**.
...
Cela cree une architecture ou les 3 blocs conv sont connectes directement a l'entree de maniere independante. Seul le 3e bloc (128 filtres) alimente le reste -- les deux premiers blocs sont completement ignores.
...
def cnn():
    ...
    # Deuxieme bloc convolutif (CORRIGE : prend layer, pas entree)
    layer = tf.keras.layers.Conv2D(
    ...
    # Troisieme bloc convolutif (CORRIGE : prend layer, pas entree)
    layer = tf.keras.layers.Conv2D(
...
| MaxPooling2D(2,2) | 28x28x32 | 14x14x32 | 0 | Reduit la resolution spatiale de moitie, conserve les activations maximales |
...
| Conv2D(128, (5,5)) | 5x5x64 | 1x1x128 | 204 928 | Detecte 128 motifs de haut niveau. Sortie 1x1 car le noyau 5x5 couvre exactement l'entree 5x5. |
...
**Explication :**
- **~78% d'accuracy** sur le test (vs ~50% pour le MLP) : une amelioration massive de +28 points.
- Les classes "cat" (3) et "dog" (5) restent les plus difficiles (similarite visuelle, confusion entre elles).
- "automobile" (1), "ship" (8), "truck" (9) sont bien classes (formes geometriques distinctives).
...
def main():
    ...
    # argmax selectionne le neurone actif (le numero de classe predit)
...
### Comparer le perceptron avec differentes tailles de couche cachee contre le CNN.
...
| Modele | Forme d'entree | Parametres | Accuracy test | Overfitting |
|--------|---------------|-----------|---------------|-------------|
...
Le MLP traite le pixel (0,0) et le pixel (31,31) comme des features independantes sans relation spatiale. Le CNN applique des filtres locaux 5x5 qui capturent les motifs de voisinage (bords, textures, formes).

**2. Partage des poids (weight sharing).**
Un filtre Conv2D est applique sur toute l'image avec les memes poids. Un filtre 5x5x3 n'a que 75 parametres mais est applique a 784 positions (28x28). Le MLP a besoin d'un poids separe pour chaque connexion pixel-neurone.
...
Le CNN a 200x moins de parametres que le MLP mais obtient de bien meilleurs resultats. Moins de parametres = moins de risque d'overfitting = meilleure generalisation.
...
Avec K=5, strides=(1,1), pas de padding : une image 32x32 produit une feature map 28x28 (32-5+1=28).
...
Fonction de cout pour la classification multiclasse avec des labels entiers :

```
L = -log(p_predite_pour_la_vraie_classe)
```

Si la vraie classe est "cat" (3) et le modele predit P(cat) = 0.2, la perte est -log(0.2) = 1.61. Si P(cat) = 0.95, la perte est -log(0.95) = 0.05. "Sparse" signifie que les labels sont des entiers, pas des vecteurs one-hot.
